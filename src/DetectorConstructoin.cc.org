...
//Another box
  solidFrame = new G4Box("Frame",
                   WorldSize*0.95/2,WorldSize*0.95/2,WorldSize*0.95/2);
  ...
  G4double vvaneT = 1.;
  G4double vvaneH = 350.;
  G4double r_vane = d_center/2+vvaneW/2;

  G4double sensorW = 222.;
  G4double sensorT = 0.32;
  G4double sensorH = 200.;
  ...
  //virtual vane
  sol_vvane = new G4Box("vvane", vvaneW/2*mm, vvaneT/2*mm, vvaneH/2*mm);
  ...
  //sensor
  sol_sensor = new G4Box("sensor", sensorW/2*mm, sensorT/2*mm, sensorH/2*mm);
  log_sensor = new G4LogicalVolume(sol_sensor,
				   sensorMat,
				   "sensor");
  phys_sensor = new G4PVPlacement(0,
				  G4ThreeVector(9.*mm,0,-75.*mm),
				 log_sensor,
				 "sensor",
				 log_vvane,
				 false,
...
//  G4double pRMinD = 5*mm;
 ...
 G4VisAttributes* sensorcol= new G4VisAttributes(G4Colour(0.8,0.8,0.8));//190, 190, 190　
 G4VisAttributes* subcol= new G4VisAttributes(G4Color(0.,0.4,0.)); // 0, 139, 69
 G4VisAttributes* tubecol= new G4VisAttributes(G4Colour(0.4,0.4,0.4)); //105, 105, 105
 log_vvane->SetVisAttributes(vvanecol);
 log_sensor->SetVisAttributes(sensorcol);
 log_sub->SetVisAttributes(subcol);
 log_sub2->SetVisAttributes(subcol);
